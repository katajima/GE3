...
void SrvManager::Initialize(DirectXCommon* dxCommon)
{
	...
	HRESULT hr = directXCommon->GetDevice()->CreateDescriptorHeap(&srvHeapDesc, IID_PPV_ARGS(&descriptorHeap));
	if (FAILED(hr)) {
		throw std::runtime_error("Failed to create descriptor heap.");
	}
	// ディスクプリタヒープの生成
	//descriptorHeap = directXCommon->CreateDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, kMaxSRVCount, true);
	// ディスクプリタヒープ1個分のサイズを取得して記録
...
void SrvManager::PreDraw()
{
	...
	ID3D12DescriptorHeap* descriporHeaps[] = { descriptorHeap.Get() };
	directXCommon->GetCommandList()->SetDescriptorHeaps(1, descriporHeaps);
}
