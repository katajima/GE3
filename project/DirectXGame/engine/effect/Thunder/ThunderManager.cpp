...
ThunderManager* ThunderManager::instance = nullptr;
...
void ThunderManager::Initialize(DirectXCommon* dxCommon)
{
	dxCommon_ = dxCommon;

	CreateGraphicsPipeline();
}
void ThunderManager::DrawCommonSetting()
{
...
void ThunderManager::Finalize()
{
	delete instance;
	instance = nullptr;
}

void ThunderManager::CreateRootSignature()
{
	HRESULT hr;
	...
	// ThunderParameters (b1) をピクセルシェーダで使用する
	rootParameters[3].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
	rootParameters[3].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
	rootParameters[3].Descriptor.ShaderRegister = 1;  // b1 とバインド
	//
...
void ThunderManager::CreateGraphicsPipeline()
{
	HRESULT hr;
	CreateRootSignature();
...
#pragma region BlendState
	...
	blendDesc.RenderTarget[0].SrcBlend = D3D12_BLEND_SRC_ALPHA; // 現在の設定
	blendDesc.RenderTarget[0].DestBlend = D3D12_BLEND_ONE;      // 雷の輝きを強調する設定例
...
#pragma endregion //BlendState(ブレンドステート)
	...
	// Shaderをコンパイルする
	Microsoft::WRL::ComPtr < IDxcBlob> vertexShaderBlob = dxCommon_->CompileShader(L"resources/shaders/Thunder/Thunder.VS.hlsl",
		L"vs_6_0");

	assert(vertexShaderBlob != nullptr);

	Microsoft::WRL::ComPtr < IDxcBlob> pixelShaderBlob = dxCommon_->CompileShader(L"resources/shaders/Thunder/Thunder.PS.hlsl",
		L"ps_6_0");
