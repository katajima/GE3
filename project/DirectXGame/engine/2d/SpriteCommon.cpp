...
void SpriteCommon::DrawCommonSetting2()
{
	// RootSignatureを設定。PSOに設定しているけど別途設定が必要
...
void SpriteCommon::CreateRootSignature2()
{
	HRESULT hr;
	...
	hr = dxCommon_->GetDevice()->CreateRootSignature(0, signatureBlob->GetBufferPointer(),
		signatureBlob->GetBufferSize(), IID_PPV_ARGS(&rootSignature2));
...
void SpriteCommon::CreateGraphicsPipeline2()
{
	HRESULT hr;
...
#pragma endregion //BlendState(ブレンドステート)
	...
	graphicsPipelineStateDesc.pRootSignature = rootSignature2.Get();// RootSignature
	...
	hr = dxCommon_->GetDevice()->CreateGraphicsPipelineState(&graphicsPipelineStateDesc,
		IID_PPV_ARGS(&graphicsPipelineState2));
